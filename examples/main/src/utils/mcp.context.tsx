...
export const MCPProvider = ({ children }: { children: ReactNode }) => {
  ...
  const connect = async () => {
    ...
    try {
      const newClient = new Client(
        { name: 'wllama-client', version: '1.0.0' },
        { capabilities: { tools: { list: true, call: true }, elicitation: {} } }
      );
      ...
      newTransport.onerror = (err: unknown) =>
  ...
  const readResource = async (uri: string) => {
    ...
    return client.request(
      ...
      ReadResourceResultSchema as any
    );
  };
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/40 z-50">
          <div className="bg-white rounded p-4 max-w-lg w-full">
            <div className="font-bold mb-2">
              {elicitationRequest.params?.message || 'Request information'}
            </div>
            <ElicitationForm
